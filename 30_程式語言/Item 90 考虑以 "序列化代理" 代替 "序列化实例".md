...
private static class SerializationProxy implements Serializable { // 私有静态嵌套类
	...
	// // 有一个构造函数，其参数类型是外围类。这个构造函数只是从它的参数复制数据：它不需要做任何一致性检查或防御性复制
...
该方法存在于外围类，导致序列化系统产生 SerializationProxy 实例，而不是外围类的实例。换句话说，writeReplace 方法會在序列化之前，将外围类的实例转换为其序列化代理。
...
最后，在 SerializationProxy 类上提供一个 readResolve 方法，该方法返回外围类的逻辑等效实例。此方法的可以讓序列化系统在反序列化时将序列化代理转换回外围类的实例。
...
这个 readResolve 方法仅使用其公共 API 创建了一个外围类的实例，这就是该模式的美妙之处。它在很大程度上消除了序列化的语言外特性(extralinguistic character)，因为反序列化实例就是使用与任何其他实例相同的构造函数、静态工厂和方法去创建的。这樣你就不必另外去确保反序列化的实例需要遵从类的不变性。只要类的静态工厂或构造函数建立了这些不变性，而它的实例方法已經有去维护不变性，那么你就已經确保了这些不变性。
...
与防御性复制方法（第 357 页）类似，序列化代理方法阻止伪字节流攻击 (bogus byte-stream attack)（第 354 页）和内部字段盗窃攻击 ( internal field theft attack)（第 356 页）。与前两种方法不同，这种方法不需要權衡，可以允许 Period 的字段为 final，这可以讓 Period 类可以成為不可变類(也是我們希望的)（[Item-17](/Chapter-4/Chapter-4-Item-17-Minimize-mutability.md)）。与前两种方法不同，这一种方法不需要太多的思考。你不必指出哪些字段可能会受到狡猾的序列化攻击的危害，也不必显式地执行有效性检查作为反序列化的一部分。
...
考虑 EnumSet 的情况（[Item-36](/Chapter-6/Chapter-6-Item-36-Use-EnumSet-instead-of-bit-fields.md)）。该类没有公共构造函数，只有静态工厂。从客户端的角度来看，它们返回 EnumSet 实例，但是在当前 OpenJDK 实现中，它们返回两个子类中的一个，那一個會具体取决于底层枚举类型的大小：如果底层枚举类型有 64 个或更少的元素，则静态工厂返回一个 RegularEnumSet；否则，它们返回一个 JumboEnumSet。
...
现在考虑，如果序列化一个枚举集合，它的枚举类型有 60 个元素，然后给这个枚举类型再增加 5 个元素，之后反序列化这个枚举集合。当它被序列化的时候是 RegularEnumSet 实例，而被反序列化時，希望是 JumboEnumSet 实例。事实上正是这样，因为 EnumSet 使用序列化代理模式。如果你好奇，这里是 EnumSet 的序列化代理。其实很简单：
...
譯註：
1. **序列化 (EnumSet 物件):**
    - 當 EnumSet 物件被序列化時，它的 writeReplace() 方法（EnumSet 內部有實作）會返回一個 SerializationProxy 物件。
    - Java 序列化機制會序列化這個 SerializationProxy 物件，而不是直接序列化 EnumSet 物件。
    - SerializationProxy 物件會儲存 EnumSet 的元素型別和所有元素。
2. **反序列化:**
    - 當反序列化 SerializationProxy 物件時，Java 序列化機制會建立 SerializationProxy 物件。
    - 反序列化後，JVM 會調用 SerializationProxy 物件的 readResolve() 方法。
    - readResolve() 方法會根據儲存的元素型別和元素建立一個新的 EnumSet 物件，並返回這個 EnumSet 物件。
    - 最終，反序列化過程返回的是一個重建的 EnumSet 物件，而不是 SerializationProxy 物件。
...
序列化代理模式有两个限制。它不兼容於哪些客户端可以扩展的类（[Item-19](/Chapter-4/Chapter-4-Item-19-Design-and-document-for-inheritance-or-else-prohibit-it.md)）。而且，如果類的bject graphs有循環，那也不兼容：如果你试图从对象的序列化代理的 readResolve 方法中调用对象上的方法，你将得到一个 ClassCastException，因为你还没有对象，只有对象的序列化代理。
