...
你可以在指定位置进行更正，使警告消失。注意，你实际上不必指定类型参数，只需给出由 Java 7 中引入的 diamond 操作符（<>）。然后编译器将推断出正确的实际类型参数：
...
总之
- unchecked 警告很重要。不要忽视他们。
- 每个 unchecked 警告都代表了在运行时发生 ClassCastException 的可能性。
- 尽最大努力消除这些警告。
- 如果不能消除 unchecked 警告，并且可以证明引发该警告的代码是类型安全的，那么可以在尽可能狭窄的范围内使用 @SuppressWarnings("unchecked") 注释来禁止警告。
- 在注释中记录你决定隐藏警告的理由。
