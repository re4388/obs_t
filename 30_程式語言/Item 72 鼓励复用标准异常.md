...
另一个常被复用异常是 IllegalStateException。如果接收对象的状态导致调用非法，则通常会抛出此异常。例如，如果调用者试图在对象被初始化之前使用它，那么这将是抛出的异常。
...
另一个可复用异常是 ConcurrentModificationException。如果一个对象被设计为由单个线程使用（或透過外部的同步），并且检测到它正在被并发地修改，则应该抛出该异常。因为不可能可靠地检测并发修改，所以该异常充其量只是一个提示。
...
譯註:
序列化問題： 異常類是可序列化的。這意味著異常對象的狀態可以被保存到磁碟或通過網絡傳輸。 如果你創建了自定義異常類，你需要仔細考慮序列化和反序列化的問題。 例如，你需要確保異常類的版本兼容性，避免在反序列化時出現 InvalidClassException 等問題。 使用標準庫的異常類可以避免這些麻煩，因為它們已經妥善處理了序列化問題。
...
选择复用哪个异常可能比较棘手，因为上表中的「使用场合」似乎并不相互排斥。考虑一个对象，表示一副牌，假设有一个方法代表发牌操作，该方法将手牌多少作为参数。如果调用者传递的值大于牌堆中剩余的牌的数量，则可以将其解释为 IllegalArgumentException （handSize 参数值太大）或 IllegalStateException（牌堆中包含的牌太少）。在这种情况下，规则是：如果是没有提供参数值的情況，抛出 IllegalStateException，如果是錯誤的參數，抛出 IllegalArgumentException。