...
**如果您使用延迟初始化来取代初始化 circularity，请使用同步访问器**(synchronized accessor)，因为它是最简单、最清晰的替代方法：
...
这两种习惯用法（一般初始化 和 使用同步访问器进行延迟初始化）在应用在 静态字段时都一樣，只是在字段和访问器声明中添加了 static 修饰符。
...
**如果需要在静态字段上使用延迟初始化来提高性能，use the lazy initialization holder class idiom.** 这个寫法利用了"一個類只有被使用才初始化"這一點。
...
如果需要使用延迟初始化在 "實例字段" 且重視性能，请使用双重检查模式。这个模式避免了初始化后访问字段时的锁定成本（[Item-79](/Chapter-11/Chapter-11-Item-79-Avoid-excessive-synchronization.md)）。这个模式背后的思想是两次检查字段的值（因此得名 double check）：一次没有锁定，然后，如果字段没有初始化，第二次再使用锁定。只有当第二次检查指示字段未初始化时，调用才初始化字段。**由于初始化字段后没有锁定，因此将字段声明为 volatile 非常重要**（[Item-78](/Chapter-11/Chapter-11-Item-78-Synchronize-access-to-shared-mutable-data.md)）。下面是这个模式的示例：
...
private FieldType getField() {
    ...
    if (result == null) { // First check (no locking)
        ...
        synchronized(this) {
            if (field == null) // Second check (with locking via synchronized)
...
虽然不是严格必需的，但这可能会提高性能，而且与 low-level 并发编程相比，这更优雅。在我的机器上，上述方法的速度大约是没有局部变量版本的 1.4 倍。虽然您也可以将双重检查模式应用于静态字段，但是没有理由这样做，因為 the lazy initialization holder class idiom is a better choice.
...
双重检查模式的两个变体值得注意。有时候，您可能需要延迟初始化一个实例字段，但该字段可以容忍重复初始化。如果您发现自己处于这种情况，您可以使用双重检查模式的变体来避免第二个检查獲得更好的效能。毫无疑问，这就是所谓的「单检查」模式。它是这样的。注意，field 仍然声明为 volatile：
...
private FieldType getField() {
    // 將字段的值讀取到局部變量 result 中。 這一步很重要，因為它可以避免在 if 條件中多次讀取 field 的值
    FieldType result = field;
    if (result == null)
        // 初始化字段，并将结果赋给局部变量 result。 由於沒有第二次檢查，因此可能會有多個線程同時執行這一步。
        // 由於沒有第二次檢查，因此可能會有多個線程同時進行初始化。 但由於字段可以容忍重複初始化，因此不會造成問題。只是會浪費一些計算資源。
        field = result = computeFieldValue();
    return result;
}
```
譯註： 只適用於可以容忍重複初始化的場景。 如果字段的初始化操作有副作用，或者初始化成本非常高，則不應該使用單檢查模式。
...
本条目中讨论的所有初始化技术都适用于基本字段(primitive fields)和对象引用字段(object reference fields)。当双检查或单检查模式应用于数值基本类型字段时，将根据 0（数值基本类型变量的默认值）而不是 null 检查字段的值。
...
如果您不关心每个线程是否都会重新计算字段的值，并且字段的类型**是 long 或 double 之外**的基本类型，那么您可以选择在单检查模式中从字段声明中删除 volatile 修饰符。这种变体称为原生单检查模式(racy single-check idiom)。它加快了某些架构上的字段访问速度，代价是需要额外的初始化（每个访问该字段的线程最多需要一个初始化）。这绝对是一种奇特的技术，不是日常使用的。
...
- 如果必须延迟初始化字段以实现性能目标或 break a harmful initialization circularity (循環依賴)，则使用适当的延迟初始化技术。
- 对于字段，使用双重检查模式；
- 对于静态字段，the lazy initialization holder class idiom. 
- 可以容忍重复初始化的实例字段，您还可以考虑单检查模式(single-check idiom)。