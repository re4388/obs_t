...
这个案例的寓意很简单：**顾名思义，异常只适用于确有异常的情况；它们不应该用于一般的控制流程。** 更进一步说，使用标准的、易于识别的习惯用法，而不是声称能够提供更好性能的"過度机灵的技巧"。即使性能优势是真实存在的，但在稳步改进平台实现的前提下，这种优势也并不可靠。而且，這種"過度机灵的技巧"所存在的细微缺陷和维护问题肯定会继续存在。
...
这个原则对 API 设计也有其意涵。一个设计良好的 API 不能迫使其客户端为一般的控制流程去處理异常。只有在某些不可预知的条件下才能调用具有「状态依赖」方法的类，通常应该有一个单独的「状态测试」方法，表明是否适合调用「状态依赖」方法。例如，Iterator 接口具有「状态依赖」的 next 方法和对应的「状态测试」方法 hasNext。这使得传统 for 循环（在 for-each 循环内部也使用了 hasNext 方法）在集合上进行迭代成为标准习惯用法：
...
提供单独的「状态测试」方法 (like `Iterator.hasNext`)的另一种方式，就是让「状态依赖」方法 (like `Iterator.next`) 返回一个空的 Optional 对象（[Item-55](/Chapter-8/Chapter-8-Item-55-Return-optionals-judiciously.md)），或者在它不能执行所需的计算时返回一个可识别的值，比如 null。
...
有一些指导原则，帮助你在「状态测试」方法(`Iterator.hasNext`)、Optional、可识别的返回值(like `null` or `-`1 之類的) 之间进行选择。
（1）如果要在没有外部同步的情况下并发地访问对象，或者受制于外部条件的状态转换，则必须使用 Optional 或可识别的返回值，因为对象的状态可能在调用「状态测试」方法`Iterator.hasNext`与「状态依赖」方法 (`Iterator.next` )的间隔中发生变化。
...
（4）如果你忘记检查一个可识别的返回值(like `null` or `-`1 之類的)，那么这个 bug 可能很难发现。但是这对于返回 Optional 对象的方式来说不是问题，因為後者會明確要求使用者要進行處理。
...
总之，异常是为确有异常的情况设计的。不要将它们用于一般的控制流程，也不要設計這種會強迫使用者这样做的 API。