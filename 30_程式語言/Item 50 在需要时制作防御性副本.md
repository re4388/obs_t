...
Java 是一种安全的语言，这是它的一大优点。这意味着在没有本地方法(native methods)的情况下，它不受缓冲区溢出、数组溢出、非法指针和其他内存损坏错误的影响，这些错误困扰着 C 和 C++ 等不安全语言。在一种安全的语言中，可以编写一个类并确定它们的不变量将保持不变，而不管在系统的任何其他部分发生了什么。這在将所有内存视为一个巨大数组的语言中是不可能的。
...
还要注意，我们没有使用 Date 的 clone 方法来创建防御性副本。因为 Date 不是 final 的，所以不能保证 clone 方法返回一个 java.util.Date 的实例对象：它可以返回一个不受信任子类的实例，这个子类是专门为恶意破坏而设计的。例如，这样的子类可以在创建时在私有静态列表中记录对每个实例的引用，并允许攻击者访问这个列表。这将使攻击者可以自由控制所有实例。为防止此类攻击，**对於不受信任方的子类化的参数类型，不要使用 clone 方法进行防御性复制。**
[[对于来自不受信任方的、非final 类的对象（例如Date），不要使用clone() 方法进行防御性复制]]
...
譯註:
构造函数: 构造函数用于创建新的对象，其参数通常来自外部，也就是不受信任的客户端代码。 正如之前解释的，如果参数是一个非final 类的对象（例如Date），客户端代码可以传入一个恶意子类的实例，其clone() 方法可能被覆盖，从而导致安全风险。

访问器方法 (getter): 访问器方法用于返回对象的内部状态。 在这个例子中，Period 类内部维护了一个java.util.Date 对象。由于这个Date 对象是由Period 类自己创建和管理的，而不是来自外部不受信任的代码，因此可以确定它的类型确实是java.util.Date，而不是某个恶意子类。 因此，在访问器方法中使用clone() 方法创建Date 对象的副本是相对安全的。
...
总而言之
- 如果一个类具有可變組件，然後這個組件可以从客户端获取，or/and 可以返回给客户端。则该类必须防御性地复制这些组件。
- 如果复制的成本过高，并且类信任它的客户端不会不适当地修改组件，那么可以不进行防御性的复制，然後要在文档中指明客户端的职责是不得修改會受到客戶影响的组件。
