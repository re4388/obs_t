...
- 最后一个例子是，在 API 中使用实现类而不是接口将你绑定到特定的实现，即使将来可能会编写更快的实现也无法使用（[Item-64](/Chapter-9/Chapter-9-Item-64-Refer-to-objects-by-their-interfaces.md)）。
...
API 设计对性能的影响是非常实际的。考虑 `java.awt.Component` 中的 getSize 方法。對這個性能很关键方法返回 Dimension 实例的這個决定，再加上维度实例是可变的，因此该方法的任何实现在每次调用时會分配一个新的 Dimension 实例。尽管在现代 VM 上分配小对象并不昂贵，但不必要地分配数百万个对象也会对性能造成实际损害。
...
存在几种 API 设计替代方案。理想情况下，Dimension 应该是不可变的（[Item-17](/Chapter-4/Chapter-4-Item-17-Minimize-mutability.md)）；或者，getSize 可以使用返回 Dimension 对象的原始组件的两个方法所替代。事实上，出于性能原因，在 Java 2 的组件中已經添加了两个这样的方法。然而，现有的客户端代码(除非修改)，不然仍然必須使用 getSize 方法，并且仍然受到原始 API 设计决策的性能影响。
...
幸运的是，通常情况下，好的 API 设计与好的性能是一致的。 **为了获得良好的性能而調整 API 是一个非常糟糕的想法。** 导致你改变 API 的性能问题，可能在平台或其他底层软件的未来版本中消失，但是改变的 API 和随之而来的问题将永远伴随着你。
