...
此外，pop 方法抛出的 ArrayIndexOutOfBoundsException 也不适于高层抽象解释（[Item-73](/Chapter-10/Chapter-10-Item-73-Throw-exceptions-appropriate-to-the-abstraction.md)）。(譯註:上例有進行異常抽象轉換)
...
实现故障原子性的另一种方式是对计算的程式进行順序的調整，以便可能发生故障的部分都先于修改对象的部分发生。这种方法是前一种方法的自然扩展。例如，考虑 TreeMap 的情况，它的元素按照一定的顺序排序。为了向 TreeMap 中添加元素，元素的类型必须能够使用 TreeMap 的顺序进行比较，在以任何方式修改「树」之前，试图添加类型不正确的元素自然会失败，并导致 ClassCastException 异常。
...
**实现故障原子性的第三种方法是以对象的临时副本(a temporary copy)执行操作，并在操作完成后用临时副本替换对象的内容。当数据存储在临时数据结构中后，计算过程如果會會更加迅速，使用這種方法就是很自然的作法。** 例如，一些排序函数在排序之前将其输入 list 复制到数组中，以降低访问排序内循环中的元素的成本。这样做是为了提高性能，但是作为一个额外的好处，它确保如果排序失败，输入 list 将保持不变。
...
虽然故障原子性通常是可取的，但它并不总是可以实现的。例如，如果两个线程试图在没有适当同步的情况下并发地修改同一个对象，那么该对象可能会处于不一致的状态。因此，如果假定在捕捉到 ConcurrentModificationException 之后，对象仍然可以被使用，那就错了。该错误是不可恢复的，所以在抛出 AssertionError 时，你甚至不需要尝试保存故障原子性。
...
即使在可以实现故障原子性的情况下，也并不总是可取的。对于某些操作，它将显著增加成本或复杂性。

總之，一旦意识到这个问题，通常是可以轻松地实现故障原子性。
...
結論：
- 作为规则，也作为方法规范的一部分，生成的任何异常都不應該改變对象的狀態，狀態在方法调用前後應該一樣。aka: 故障原子性
- 如果不幸的违反了这条规则，API 文档应该清楚地指出对象将处于什么状态。
- 不幸的是，许多现有的 API 文档都没有做到。