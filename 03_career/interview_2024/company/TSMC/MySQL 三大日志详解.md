...
## [前言](https://javaguide.cn/database/mysql/mysql-logs.html#%E5%89%8D%E8%A8%80)

MySQL 日志 主要包括错误日志、查询日志、慢查询日志、事务日志、二进制日志几大类。其中，比较重要的还要属二进制日志 binlog（归档日志）和事务日志 redo log（重做日志）和 undo log（回滚日志）。

![[100_attachements/9a69f7792e55e3555f586f03b168d411_MD5.png]]
...
![[100_attachements/7bd555885d281cd655fcb72e5f414917_MD5.png]]
...
![[100_attachements/c2cf0362447a33212d63bd22570b360b_MD5.png]]
...
![[100_attachements/31a0d12425e494a2d778fb17856cd2b0_MD5.png]]
...
![[100_attachements/567acff3bd18f504e1f899f4eaf822ff_MD5.png]]

除了后台线程每秒 `1` 次的轮询操作，还有一种情况，当 `redo log buffer` 占用的空间即将达到 `innodb_log_buffer_size` 一半的时候，后台线程会主动刷盘。

下面是不同刷盘策略的流程图。

#### [innodb_flush_log_at_trx_commit=0](https://javaguide.cn/database/mysql/mysql-logs.html#innodb-flush-log-at-trx-commit-0)

![[100_attachements/af1d9aa010286c96142c4bdc6579b026_MD5.png]]

为 `0` 时，如果 MySQL 挂了或宕机可能会有 `1` 秒数据的丢失。

#### [innodb_flush_log_at_trx_commit=1](https://javaguide.cn/database/mysql/mysql-logs.html#innodb-flush-log-at-trx-commit-1)

![[100_attachements/b6c262bd0659d470e8f6f16750ed822e_MD5.png]]

为 `1` 时， 只要事务提交成功，redo log 记录就一定在硬盘里，不会有任何数据丢失。

如果事务执行期间 MySQL 挂了或宕机，这部分日志丢了，但是事务并没有提交，所以日志丢了也不会有损失。

#### [innodb_flush_log_at_trx_commit=2](https://javaguide.cn/database/mysql/mysql-logs.html#innodb-flush-log-at-trx-commit-2)

![[100_attachements/25e27b037fe0ff76082d37b3b8594061_MD5.png]]
...
![[100_attachements/23fb920ba06dfb9e23cb1895183467b5_MD5.png]]
...
![[100_attachements/19049e203ae2c50f040a64974950695b_MD5.png]]

如果 `write pos` 追上 `checkpoint` ，表示**日志文件组**满了，这时候不能再写入新的 redo log 记录，MySQL 得停下来，清空一些记录，把 `checkpoint` 推进一下。

![[100_attachements/9c5ed8d3ce678e83e4e418235da3f420_MD5.png]]
...
![[100_attachements/182b04134c766b363ff001cb14f7ec8b_MD5.png]]

可以看到刚好是 32 个，并且每个日志文件的大小是 `671088640 / 32 = 20971520`

所以在使用 MySQL 8.0.30 及之后的版本时，推荐使用 `innodb_redo_log_capacity` 变量配置日志文件组

### [redo log 小结](https://javaguide.cn/database/mysql/mysql-logs.html#redo-log-%E5%B0%8F%E7%BB%93)
...
![[100_attachements/bfd3c880db6e5c9e72b91b6ebf5983fa_MD5.png]]

binlog 会记录所有涉及更新数据的逻辑操作，并且是顺序写。

### [记录格式](https://javaguide.cn/database/mysql/mysql-logs.html#%E8%AE%B0%E5%BD%95%E6%A0%BC%E5%BC%8F)
...
![[100_attachements/587a4a08f671a344faf5f47722237d2e_MD5.png]]

同步数据时，会执行记录的 `SQL` 语句，但是有个问题，`update_time=now()` 这里会获取当前系统时间，直接执行会导致与原库的数据不一致。

为了解决这种问题，我们需要指定为 `row`，记录的内容不再是简单的 `SQL` 语句了，还包含操作的具体数据，记录内容如下。

![[100_attachements/fe0e40b831478c54e03088bdac42a7b7_MD5.png]]
...
### [写入机制](https://javaguide.cn/database/mysql/mysql-logs.html#%E5%86%99%E5%85%A5%E6%9C%BA%E5%88%B6)
...
![[100_attachements/4b2c46b09da772352e8c555c6f3950da_MD5.png]]
...
![[100_attachements/946e2926b2086734602f601b9bddef7d_MD5.png]]
...
![[100_attachements/9ff32e47fff4a29fcbef83a9add92ed4_MD5.png]]

在出现 IO 瓶颈的场景里，将 `sync_binlog` 设置成一个比较大的值，可以提升性能。

同样的，如果机器宕机，会丢失最近 `N` 个事务的 binlog 日志。

## [两阶段提交](https://javaguide.cn/database/mysql/mysql-logs.html#%E4%B8%A4%E9%98%B6%E6%AE%B5%E6%8F%90%E4%BA%A4)
...
![[100_attachements/5977b0db90dee214864745d0ff16a252_MD5.png]]
...
![[100_attachements/5ea26e6583096a80788fb3d1b2e0b8db_MD5.png]]

由于 binlog 没写完就异常，这时候 binlog 里面没有对应的修改记录。因此，之后用 binlog 日志恢复数据时，就会少这一次更新，恢复出来的这一行 `c` 值是 `0`，而原库因为 redo log 日志恢复，这一行 `c` 值是 `1`，最终数据不一致。

![[100_attachements/7dde57290933eae7925e2e7c74eb70b8_MD5.png]]

为了解决两份日志之间的逻辑一致问题，InnoDB 存储引擎使用**两阶段提交**方案。

原理很简单，将 redo log 的写入拆成了两个步骤 `prepare` 和 `commit`，这就是**两阶段提交**。

![[100_attachements/a2741b25e6db501abdbf36206cd205c6_MD5.png]]

使用**两阶段提交**后，写入 binlog 时发生异常也不会有影响，因为 MySQL 根据 redo log 日志恢复数据时，发现 redo log 还处于 `prepare` 阶段，并且没有对应 binlog 日志，就会回滚该事务。

![[100_attachements/5bceacc5f384fa1459693b9998f76c54_MD5.png]]

再看一个场景，redo log 设置 `commit` 阶段发生异常，那会不会回滚事务呢？

![[100_attachements/2203236d62b6f883e0cb99c1c8dbcef7_MD5.png]]
...
## [总结](https://javaguide.cn/database/mysql/mysql-logs.html#%E6%80%BB%E7%BB%93)
...
## [参考](https://javaguide.cn/database/mysql/mysql-logs.html#%E5%8F%82%E8%80%83)
