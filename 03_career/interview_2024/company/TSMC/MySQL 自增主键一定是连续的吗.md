...
![[100_attachements/34eb6915cdc05db910c79e10e285a0f3_MD5.png]]

## [自增值保存在哪里？](https://javaguide.cn/database/mysql/mysql-auto-increment-primary-key-continuous.html#%E8%87%AA%E5%A2%9E%E5%80%BC%E4%BF%9D%E5%AD%98%E5%9C%A8%E5%93%AA%E9%87%8C)

使用 `insert into test_pk values(null, 1, 1)` 插入一行数据，再执行 `show create table` 命令来看一下表的结构定义：

![[100_attachements/11af4dc00bbf658713c79003295f761e_MD5.png]]

上述表的结构定义存放在后缀名为 `.frm` 的本地文件中，在 MySQL 安装目录下的 data 文件夹下可以找到这个 `.frm` 文件：

![[100_attachements/8b05d8e8c16781a164a3cf3010085177_MD5.png]]
...
![[100_attachements/103e75a6ff2204596dba4bcce02fd932_MD5.png]]

但如果马上重启 MySQL 实例，重启后这个表的 AUTO_INCREMENT 就会变成 1。﻿ 也就是说，MySQL 重启可能会修改一个表的 AUTO_INCREMENT 的值。

![[100_attachements/1bab6311112406f56bee76aaa26bccd3_MD5.png]]

![[100_attachements/bab61a3fc2575c100808e6c20081f9bd_MD5.png]]
...
## [自增值不连续的场景](https://javaguide.cn/database/mysql/mysql-auto-increment-primary-key-continuous.html#%E8%87%AA%E5%A2%9E%E5%80%BC%E4%B8%8D%E8%BF%9E%E7%BB%AD%E7%9A%84%E5%9C%BA%E6%99%AF)

### [自增值不连续场景 1](https://javaguide.cn/database/mysql/mysql-auto-increment-primary-key-continuous.html#%E8%87%AA%E5%A2%9E%E5%80%BC%E4%B8%8D%E8%BF%9E%E7%BB%AD%E5%9C%BA%E6%99%AF-1)
...
### [自增值不连续场景 2](https://javaguide.cn/database/mysql/mysql-auto-increment-primary-key-continuous.html#%E8%87%AA%E5%A2%9E%E5%80%BC%E4%B8%8D%E8%BF%9E%E7%BB%AD%E5%9C%BA%E6%99%AF-2)

举个例子，我们现在往表里插入一条 (null,1,1) 的记录，生成的主键是 1，AUTO_INCREMENT= 2，对吧

![[100_attachements/7ee2b6c3664943990870bc2cdf43fedb_MD5.png]]

这时我再执行一条插入 `(null,1,1)` 的命令，很显然会报错 `Duplicate entry`，因为我们设置了一个唯一索引字段 `a`：

![[100_attachements/e3ddbc350f7f0b1057704ecaa8637370_MD5.png]]
...
### [自增值不连续场景 3](https://javaguide.cn/database/mysql/mysql-auto-increment-primary-key-continuous.html#%E8%87%AA%E5%A2%9E%E5%80%BC%E4%B8%8D%E8%BF%9E%E7%BB%AD%E5%9C%BA%E6%99%AF-3)

我们现在表里有一行 `(1,1,1)` 的记录，AUTO_INCREMENT = 3：

![[100_attachements/0e65ba7bb828b5ae4fe77a8362c01a94_MD5.png]]

我们先插入一行数据 `(null, 2, 2)`，也就是 (3, 2, 2) 嘛，并且 AUTO_INCREMENT 变为 4：

![[100_attachements/8b096109caf3b9f2a7f823117883bddc_MD5.png]]

再去执行这样一段 SQL：

![[100_attachements/f96a425e68e1f2016da7a086505bb2c4_MD5.png]]

虽然我们插入了一条 (null, 3, 3) 记录，但是使用 rollback 进行回滚了，所以数据库中是没有这条记录的：

![[100_attachements/161083b0931bfb8296f5d0c04f4807de_MD5.png]]

在这种事务回滚的情况下，自增值并没有同样发生回滚！如下图所示，自增值仍然固执地从 4 增加到了 5：

![[100_attachements/8443c4595b964bb7073ad323b107d135_MD5.png]]

所以这时候我们再去插入一条数据（null, 3, 3）的时候，主键 id 就会被自动赋为 `5` 了：

![[100_attachements/6872d20cfa1c5275e561843ff9fe816c_MD5.png]]
...
![[100_attachements/f12447eac51cb893e31caba568f157d5_MD5.png]]
...
### [自增值不连续场景 4](https://javaguide.cn/database/mysql/mysql-auto-increment-primary-key-continuous.html#%E8%87%AA%E5%A2%9E%E5%80%BC%E4%B8%8D%E8%BF%9E%E7%BB%AD%E5%9C%BA%E6%99%AF-4)
...
![[100_attachements/6179ba1543ae528b05ce3501c17faeb6_MD5.png]]

我们创建一个和当前表 `test_pk` 有相同结构定义的表 `test_pk2`：

![[100_attachements/5139cf74695ae4925d88291aaf0c1efd_MD5.png]]

然后使用 `insert...select` 往 `teset_pk2` 表中批量插入数据：

![[100_attachements/adeeed48c3f8664dbf26038a254ca403_MD5.png]]

可以看到，成功导入了数据。

再来看下 `test_pk2` 的自增值是多少：

![[100_attachements/a38d2590fe26ddd3552169e6d04f3cf6_MD5.png]]
...
![[100_attachements/52cb6802d96fab7d1d44f2bd7e2feecf_MD5.png]]

## [小结](https://javaguide.cn/database/mysql/mysql-auto-increment-primary-key-continuous.html#%E5%B0%8F%E7%BB%93)
